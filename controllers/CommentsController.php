...
class CommentsController extends Controller
{
    ...
    public function actionShowModal()
    {
        ...
        try
        {
            ...
            if($request->isAjax)
            {
                ...
                {
                    $action = $request->post("modalAction");
                }
    ...
    public function actionAuth()
    {
        ...
        try
        {
            if($request->isAjax && $request->post("loginData"))
            {
                ...
                $transform = array(
                    0 => $request->post("loginData")[1]["value"],
                    1 => $request->post("loginData")[2]["value"]
                );

                if(emptyCheck($transform) === true && ActiveForm::validate($model))
                {
                    $email = $request->post("loginData")[1]["value"];
                    $password = $request->post("loginData")[2]["value"];
    ...
    public function actionSort()
    {
        ...
        try
        {
            ...
            {
                $orderBy = $request->post("sortData")["parametr"];
                $sortType = constant('SORT_' . $request->post("sortData")["sortType"]);
    ...
    public function actionCommentAdd()
    {
        ...
        try
        {
            if($request->isAjax && $request->post("postData"))
            {
                ...
                $commentBody = $request->post("postData")[1]["value"];
    ...
    public function actionDelEdit()
    {
        ...
        try
        {
            ...
            {
                $commentBody = '';
                $action = $request->post("actionData")["commentAction"];
                $commentID = $request->post("actionData")["commentNamespace"];

                if(isset($_POST["actionData"]["commentBody"]))
                {
                    $commentBody = $request->post("actionData")["commentBody"];
                }
    ...
    public function actionUserRegistration()
    {
        ...
        try
        {
            ...
            {
                $transform = array(
                    0 => $request->post("registrationData")[1]["value"],
                    1 => $request->post("registrationData")[2]["value"],
                    2 => $request->post("registrationData")[3]["value"],
                    3 => $request->post("registrationData")[4]["value"]
                );

                if(emptyCheck($transform) === true && ActiveForm::validate($model))
                {
                    $email = $request->post("registrationData")[1]["value"];
                    ...
                    {
                        $password = $request->post("registrationData")[2]["value"];
                        $password = md5($password);
                        $firstName = $request->post("registrationData")[3]["value"];
                        $secondName = $request->post("registrationData")[4]["value"];
